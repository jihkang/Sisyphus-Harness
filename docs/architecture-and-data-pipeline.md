...
profile/request/receipt/evaluation binding을 fenced publication 경계에서 재검사한 후
`TaskOutcome`을 한 번만 기록한다. queue의 `completed`는 실행 종료이고
`TaskOutcome.passed`만 의미적 성공이다. outcome은 contract, profile,
`ContractEvaluation` snapshot도 포함한다. 별도의 SQLite knowledge index는
...
| agent 결과 | `AgentResult` | `LocalCodingAgent._finish()` | CLI, attempt diagnostic, benchmark scorer |
