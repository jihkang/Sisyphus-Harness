...
| Candidate decision support | `contracts/knowledge.py`, `knowledge_graph.py`, `knowledge_mutations.py`, `knowledge_read_context.py`, `knowledge_search.py`, `knowledge_dependencies.py`, `knowledge_planning.py` | candidate-only mutation admission, revision-bound traversal, search, dependency inspection, next-step ranking |
...
| Runtime composition and adapters | `runtime.py`, `adapters/`, `provider.py`, `tools.py`, `workspace_tool_arguments.py`, `workspace_tool_paths.py`, `workspace_tool_io.py`, `workspace_tool_queries.py`, `workspace_tool_mutations.py`, `verifier.py` | trust-mode 조립, bundle/Docker 또는 explicit in-process 검증, HTTP chat completion, strict allowlisted workspace 도구 |
...
flowchart TD
    ...
    Handlers --> Knowledge["knowledge_graph.py facade"]
    Knowledge --> KnowledgeRead["search / dependencies / planning"]
    Knowledge --> KnowledgeMutation["knowledge_mutations.py"]
    KnowledgeRead --> KnowledgePort["KnowledgeIndexPort"]
    KnowledgeMutation --> KnowledgePort
    KnowledgePort --> KnowledgeIndex["infra/knowledge_index.py facade"]
    KnowledgeIndex --> KnowledgeDB["infra/knowledge_database.py"]
    KnowledgeIndex --> KnowledgeProjection["infra/knowledge_projection.py"]
    KnowledgeIndex --> KnowledgeQueries["infra/knowledge_queries.py"]
    KnowledgeProjection --> KnowledgeIntegrity["infra/knowledge_integrity.py"]
    KnowledgeQueries --> KnowledgeIntegrity
    ...
    AgentTransitions --> Tools["tools.py facade"]
    Tools --> ToolQueries["workspace_tool_queries.py"]
    Tools --> ToolMutations["workspace_tool_mutations.py"]
    ToolQueries --> ToolPaths["workspace_tool_paths.py"]
    ToolMutations --> ToolPaths
    ToolQueries --> ToolIO["workspace_tool_io.py"]
    ToolMutations --> ToolIO
...
Knowledge domain의 public facade는 `KnowledgeIndexPort`에만 의존한다. 각 read는
`knowledge_read_context.py`에서 start revision, node cache, edge cache를 한 번 만들고
search/dependency/planning service가 이를 공유한 뒤 종료 시 revision을 재검사한다.
SQLite facade는 `infra/knowledge_database.py`의 schema/transaction lifecycle을
상속하고 projection, query, strict row/integrity 검증을 별도 collaborator에 위임한다.
Dependency endpoint 확인, reachability cycle 검사와 insert는 동일한
`BEGIN IMMEDIATE` transaction 안에서 수행된다.
...
| file boundary와 mutation | `tools.py`, `workspace_tool_arguments.py`, `workspace_tool_paths.py`, `workspace_tool_io.py`, `workspace_tool_queries.py`, `workspace_tool_mutations.py`, `workspace.py` | `tests/test_tools.py`, `tests/test_workspace_tool_structure.py`, `tests/test_workspace.py` |
...
| candidate-only graph retrieval | `contracts/knowledge.py`, `knowledge_graph.py`, `knowledge_read_context.py`, `knowledge_search.py`, `knowledge_dependencies.py`, `knowledge_planning.py`, `infra/knowledge_database.py`, `infra/knowledge_projection.py`, `infra/knowledge_queries.py`, `infra/knowledge_integrity.py` | `tests/test_knowledge_*`, `tests/test_evidence_graph_architecture.py` |
