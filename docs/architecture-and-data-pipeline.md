...
`VerificationServicePort`다. host는 bundle CAS에서 요청한 `.tar`/`.json` 두 파일만
...
| bundle verifier, asset CAS와 container transport | `services/verifier.py`, `infra/verifier_assets.py`, `adapters/docker_verifier.py` | `tests/test_verifier_service.py`, `tests/test_verifier_assets.py`, `tests/test_docker_verifier.py`, `tests/test_docker_verifier_host.py` |
