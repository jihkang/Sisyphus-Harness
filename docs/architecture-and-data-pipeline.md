...
기본 direct Agent는 여전히 한 Python process에서 in-process verifier를 사용한다.
반면 queued Worker는 submit 시점의 config digest, effective policy snapshot과
content-addressed workspace bundle을 payload에 고정한다. 각 claim은 Git common-dir
아래의 disposable attempt workspace에 bundle을 materialize하고 ephemeral Git baseline을
만든 뒤 Agent를 실행하며, 원본 repository를 수정하지 않고 output bundle을 게시한다.

`BundleVerifierService`는 `VerificationServicePort` 뒤에서 bundle reference와 profile
digest를 받는 독립 service boundary다. `DockerVerifierTransport`는 non-root,
no-network, read-only root filesystem,
resource limits를 적용한다. 전체 bundle CAS 대신 요청의 digest-bound `.tar`/`.json`
두 파일만 stable-stat/no-symlink/size/SHA/reference 검증을 거쳐 실행별 read-only view로
복사하며, combined Docker stdout/stderr에도 byte limit을 둔다. 실행별 빈 staging만 쓰기
가능하게 주고 output overflow/timeout에는 process group과 CID container를 정리한다.
호스트가 receipt의 size/SHA/parser/request/bundle/profile binding을 재검증한 뒤에만
authoritative artifact root로 원자 게시한다. Compose는 view 생성기를 포함하지 않으므로
operator가 `SISYPHUS_BUNDLE_STORE`를 exact validated request pair만 든 새 directory로
준비해야 한다. 일반 CLI의 기본 verification path는 아직 in-process다.

Control-side shadow adjudication은 exact Worker output bundle을 독립 검증하고,
authoritative receipt artifact를 다시 읽은 뒤 typed observation과 immutable
`EvidenceContract`를 3값으로 평가한다. 이 결과는 아직 queue terminal state나
authoritative Task/Gap lifecycle을 변경하지 않는다. 별도의 SQLite knowledge index는
...
- evidence-contract registry, TaskOutcome/Gap closure transaction과 lifecycle
- Docker/remote verifier를 기본 final authority로 선택하는 Control composition
...
| 완료 조건 | `EvidenceContract`, selector/clause/predicate AST | operator/Control | pure evaluator와 shadow adjudication |
...
기본 `BoundedVerifier`는 model tool은 아니지만 model이 변경한 코드를 operator
계정으로 실행하므로 process sandbox가 아니다. 선택적인 `DockerVerifierTransport`는
network/read-only/capability/resource boundary와 immutable bundle materialization을
제공한다. 컨테이너에는 전체 bundle CAS 대신 요청한 archive/reference만 있는 read-only
view를, authoritative evidence root 대신 실행별 빈 staging만 준다. transport output은
합산 byte limit을 넘으면 container와 함께 fail closed하고, Control host가 inline result와
receipt artifact를 대조한 뒤 run directory를 게시한다.
`/tmp`는 noexec이고 repository test program은 별도 executable `/work` tmpfs에서
실행한다.
...
`InProcessAgentRunFactory`를 통해 Agent loop를 구동하고, 결과 tree를 별도 output
bundle로 게시한 뒤 attempt workspace를 제거한다.
...
| Evidence adjudication | output bundle, profile, immutable contract | independent verify, authoritative artifact re-read, typed normalization, 3-value evaluation | observations + `ContractEvaluation` | 현재 shadow result | future TaskOutcome authority |
...
| task success self-assertion | Agent Boolean과 독립된 output-bundle final verification + EvidenceContract |
...
1. 기본 in-process verifier는 sandbox가 아니다. untrusted code에는 optional Docker
   transport 또는 별도 OS/VM boundary가 필요하다.
2. POSIX verifier는 same-process-group descendants와 열린 output pipe를 정리하지만,
   새 session/process group으로 이동하고 모든 pipe를 닫은 descendant는 관찰할 수 없다.
...
8. Docker verifier service와 test command는 한 container UID/mount namespace를
   공유한다. host containment는 제공하지만 hidden profile/oracle confidentiality는
   별도 command sandbox가 필요하다.
...
| bundle verifier와 container transport | `services/verifier.py`, `adapters/docker_verifier.py` | `tests/test_verifier_service.py`, `tests/test_docker_verifier.py` |
| evidence 조건식과 final adjudication | `contracts/evidence_contract.py`, `evidence_contract.py`, `services/evidence_contract.py` | `tests/test_evidence_*` |
