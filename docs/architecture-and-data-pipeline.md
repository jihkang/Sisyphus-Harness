...
flowchart TD
    ...
    BundleAdapter --> Docker["adapters/docker_verifier.py"]
...
verification은 현재 candidate tree를 immutable bundle로 캡처한 뒤 Docker service에서
...
- Docker/remote receipt만 받아들이는 Control-owned semantic outcome transaction
...
`BoundedVerifier`는 Docker service 내부의 command runner이며 explicit
`trusted-in-process`와 benchmark/evolution compatibility path에서는 operator 계정으로
직접 실행되므로 그 경로 자체는 process sandbox가 아니다. 기본 direct/queued
composition의 `DockerVerifierTransport`는 network/read-only/capability/resource
boundary와 immutable bundle materialization을 제공한다. 컨테이너에는 전체 bundle CAS
대신 요청한 archive/reference만 있는 read-only view를, authoritative evidence root 대신
실행별 빈 staging만 준다. transport output은 합산 byte limit을 넘으면 container와 함께
fail closed하고, host가 inline result와 receipt artifact를 대조한 뒤 run directory를
게시한다.
