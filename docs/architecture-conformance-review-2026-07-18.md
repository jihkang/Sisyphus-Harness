...
또한 bundle materialization은 의도적으로 `.git`을 포함하지 않지만 현재
`BoundedVerifier`의 mutation detection은 유효한 Git HEAD가 있는
`snapshot_workspace()`에 의존한다. 따라서 path argument만 bundle destination으로
바꾸는 방식으로는 연결할 수 없다. tree hash 기반 verifier snapshot adapter 또는
ephemeral Git initialization이 먼저 필요하다.
...
| 4 | bundle tree hash와 호환되는 Verifier mutation adapter를 추가하고 Agent-Verifier request에 연결 | primitive 완료, wiring 미구현 |
