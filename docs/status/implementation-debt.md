...
| ID | Priority | Status | Current fact | Exit condition | Slice |
| --- | --- | --- | --- | --- | --- |
| `SH-ARCH-001` | P1 | `AMBER` Partial | The working CLI slice reduces `cli.py` to a 43-line compatibility facade and separates parser, explicit dispatch, six handler groups, bounded input, policy selection, result, and rendering; local parser/route/facade tests cover all 25 commands, but current-head delivery evidence is pending. | The separated boundary passes current-head CI, merges independently, and records exact PR, CI, and merge evidence without changing CLI wire output or exit behavior. | D |
