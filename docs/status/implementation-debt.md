...
| --- | --- | --- | --- | --- | --- |
| `SH-VERIFY-001` | P0 | `RED` Non-conformant | Final Control verification is contained, but verifier-only oracle assets and verifier image/command-asset identity are not fully bound into `TaskOutcome`. | A candidate cannot read or mutate hidden oracle assets; image, profile, command assets, request, receipt, output bundle, and outcome digests are transitively bound and tamper-tested. | C |
...
| `SH-ARCH-002` | P1 | `AMBER` Partial | Logical Agent, Verifier, Evolve, and Control boundaries exist, but several run in one package/process and do not have authenticated transport or Compose parity. | Each service owns only its contracts/artifacts, communicates through explicit ports/transports, and passes in-process versus Compose contract tests. | D |
...
| `SH-SUPPLY-001` | P2 | `AMBER` Partial | Offline wheel build and pinned development tools are tested, but release tags, SBOM/provenance, image digest pinning, and dependency update policy are incomplete. | A tagged release publishes wheel/source hashes, SBOM and provenance, pins runtime image identity, and reproduces an offline install/build from the release inputs. | F |
