...
| ID | Priority | Status | Current fact | Exit condition | Slice |
| --- | --- | --- | --- | --- | --- |
| `SH-ARCH-002` | P1 | `AMBER` Partial | PR #13 at `a3a0121` separated Docker runtime, staging, host assembly, and publication; CI run `29915544947` passed all five jobs. PR #15 at `59f178e` separated the Agent loop; CI run `29917555768` passed all five jobs. PR #19 at `ea9d556` separated graph mutation/read/query services and SQLite lifecycle/projection/query/integrity behind 85- and 64-line facades with exact output parity; CI run `29921307245` passed all five jobs. PR #21 at `06cce47` separated workspace tool arguments, path policy, bounded IO, queries, and mutations behind a 90-line facade; CI run `29923115539` passed all five jobs. Agent, Verifier, Evolve, and Control still share one distribution and several process/composition roots; authenticated transport and in-process versus Compose parity are incomplete. | Each service owns only its contracts/artifacts, communicates through explicit ports/transports, and passes in-process versus Compose contract tests without weakening the single host evidence-publication authority. | D |
| `SH-IO-001` | P1 | `AMBER` Partial | Bundle and receipt paths have stable-stat/no-symlink readers, and WorkspaceTools now isolates bounded UTF-8/hash/atomic IO from path and command policy. Strict operator JSON and other call sites still do not share one race-resistant primitive, and WorkspaceTools does not yet use descriptor-relative no-follow/stable-stat reads. | One bounded file API provides containment, no-follow open, stable stat, size/digest checks, strict parsing, atomic replace, directory fsync, and adversarial race/symlink tests. | D-E |
